...
describe('OrderService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers:[OrderService,{provide:RepositoriesService,useClass:RepositoriesServiceMock}],
    }).compile();
  ...
  it('should be create order', async () => {
    service['repositoriesService']['order']['findFirst'] = jest.fn().mockResolvedValue(undefined);
  ...
  it('should be update order', async () => {
    const result = await service.update(orderEntityMock.orderId,createOrderDtoMock);
    expect(result).toEqual(orderEntityMock);
  });
  ...
  it("should be throw error if order isn't created", async () => {  
    try {
      service['repositoriesService']['order']['findFirst'] = jest.fn().mockResolvedValue(undefined);
      service['repositoriesService']['order']['create'] = jest.fn().mockRejectedValue(new Error("error"))
      await service.create(createOrderDtoMock);
    } catch (error) {
  ...
  it("should be throw error if findById return undefined", async () => {  
    try {
      service['repositoriesService']['order']['findUnique'] = jest.fn().mockResolvedValue(undefined);
      await service.findById(orderEntityMock.orderId);
    } catch (error) {
      expect(error).toBeInstanceOf(NotFoundException)
      expect(error.message).toEqual(`OrderId : ${orderEntityMock.orderId} not found`)
    }
  } )

  it("should be throw error if update return error", async () => {  
    try {
      service['repositoriesService']['order']['update'] = jest.fn().mockRejectedValue(new Error("error"))
  ...
  it("should be throw error if delete return error", async () => {  
    try {
      service['repositoriesService']['order']['delete'] = jest.fn().mockRejectedValue(new Error("error"))
