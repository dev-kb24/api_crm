...
describe("Testing Users", () => {
  ...
  describe("signup user", () => {
    ...
    it("/users/signup (POST) - throw error (internalServer)", async () => {
      ...
      jest.spyOn(repository.users, "create").mockRejectedValue(new Error("internal server error"));
      const result = await request(app.getHttpServer())
      .post("/users/signup")
      .send(user)
  ...
  describe("signin user", () => {
    ...
    it("/users/signin (POST) - throw error (internal server)", async () => {
      ...
      jest.spyOn(repository.users, "findFirst").mockRejectedValue(new Error("internal server error"));
      const result = await request(app.getHttpServer())
      .post("/users/signin")
      .send(user)
      expect(result.status).toEqual(500);
  ...
  describe("find user", () => {
    ...
    it("/users/getProfil/:id (GET) - throw error (internal server)", async () => {
      jest.spyOn(repository.users, "findUnique").mockRejectedValue(new Error("internal server error"));
      const result = await request(app.getHttpServer())
      .get(`/users/getProfil/${userId}`)
      .set("Authorization",`Bearer ${token}`)
      expect(result.status).toEqual(500);
  ...
  it('/users/password/:id (PUT) - throw error (internal server)', async () => {
    ...
    jest.spyOn(repository.users, "update").mockRejectedValue(new Error("internal server error"));
    const result = await request(app.getHttpServer())
    .put(`/users/password/${userId}`)
    .set("Authorization",`Bearer ${token}`)
    .send(updatepassword)
  ...
  describe("validation and access", () => {
    ...
    it("/users/validation/:id (Validation Mail) -  throw error (bad request)", async () => {
      ...
      expect(result.body.message).toEqual("Votre code de confirmation n'est pas le bon");
    });
  ...
  describe("delete user", () => {
    ...
    it("/users/:id (DELETE) - throw error (internal server)", async () => {
      jest.spyOn(repository.users, "delete").mockRejectedValue(new Error("internal server error"));
      const result = await request(app.getHttpServer())
      .delete(`/users/${userId}`)
      .set("Authorization",`Bearer ${token}`)
      expect(result.status).toEqual(500);
