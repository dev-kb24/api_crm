...
Then('Je devrais recevoir un statu {int} et eventuellement des messages d\'erreurs {string}', function (status,messages) {
    if(this.response){
      expect(this.response.status).toBe(status);
    }else{
      const listMessages = messages.split(',')
      expect(this.error.status).toBe(status);  
      expect(listMessages.every((message: string) => this.error.data.message.includes(message))).toBeTruthy()
